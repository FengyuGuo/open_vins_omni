...
int main(int argc, char **argv) {
  ...
  nh->param<std::string>("path_bag", path_to_bag, "/home/guo/dataset/seeker/meeting.bag");
  ...
  for (int i = 0; i < 1; i++) {
    ...
    cam_ptr->set_value(cam_calib);
    cameras.insert({i, cam_ptr});
  }
  ...
  for(int u = 0; u < IMG_WIDTH; u++)
  {
    for(int v = 0; v < IMG_HEIGHT; v++)
    {
      float u_n = (u - cx) / f;
      float v_n = (v - cy) / f;
      Eigen::Vector3f uv_norm(u_n, v_n, 1.0);
      uv_norm /= uv_norm.norm();
      Eigen::Vector3f uv_rot = rot * uv_norm;

      Eigen::Vector2f uv_dist = cam_ptr->distort_space(uv_rot);
