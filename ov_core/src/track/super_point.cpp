...
bool SuperPoint::build() {
    ...
    // auto builder = TensorRTUniquePtr<nvinfer1::IBuilder>(&spl_);
    if (!builder) {
        tensorrt_log::gLogVerbose << "corrupt in " << __FILE__ << ":" << __LINE__ << std::endl;
        return false;
    }
    ...
    auto constructed = construct_network(builder, network, config, parser); //construct network
    if (!constructed) {
        tensorrt_log::gLogVerbose << "corrupt in " << __FILE__ << ":" << __LINE__ << std::endl;
        return false;
    }
...
bool SuperPoint::construct_network(TensorRTUniquePtr<nvinfer1::IBuilder> &builder,
    ...
    tensorrt_log::gLogVerbose << "parse onnx from " << super_point_config_.onnx_file << std::endl;
...
bool SuperPoint::infer(const cv::Mat &image, Eigen::Matrix<double, 259, Eigen::Dynamic> &features) {
    tensorrt_log::gLogVerbose << "start superpoint infer";
    if (!context_) {
    ...
    tensorrt_log::gLogVerbose << "create execution context finished" << std::endl;
    ...
    tensorrt_log::gLogVerbose << "set binding dim params: " << input_index << ", 1, 1," << image.rows << ", " << image.cols << std::endl;
    context_->setBindingDimensions(input_index, Dims4(1, 1, image.rows, image.cols));

    BufferManager buffers(engine_, 0, context_.get());
    tensorrt_log::gLogVerbose << "create trt buffer." << std::endl;
    ASSERT(super_point_config_.input_tensor_names.size() == 1);
    if (!process_input(buffers, image)) {
        return false;
    }
    buffers.copyInputToDevice();
    // tensorrt_log::gLogVerbose << "Sleeping for 500 milliseconds..." << std::endl;
    // std::this_thread::sleep_for(std::chrono::milliseconds(500));

    tensorrt_log::gLogVerbose << "copy data to device before inference" << std::endl;
        

    bool status = context_->executeV2(buffers.getDeviceBindings().data());
    if (!status) {
        tensorrt_log::gLogVerbose << __FILE__ << ":" << __LINE__ << ":inference failed!" << std::endl;
        return false;
    }
...
bool SuperPoint::process_input(const BufferManager &buffers, const cv::Mat &image) {
    ...
    tensorrt_log::gLogVerbose << "process input success." << std::endl;
    return true;
}
...
void SuperPoint::save_engine() {
    ...
    if (engine_ != nullptr) {
        ...
        tensorrt_log::gLogVerbose << "save engine to " << super_point_config_.engine_file << std::endl;
        if (!file) return;
