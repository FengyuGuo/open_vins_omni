...
bool SuperGlue::build() {
    ...
    // std::unique_ptr<Int8EntropyCalibrator> calibrator = std::make_unique<Int8EntropyCalibrator>("calibration_data.bin");
    ...
    tensorrt_log::gLogVerbose << "start to set optimization profile" << std::endl;
    ...
    profile->setDimensions(superglue_config_.input_tensor_names[5].c_str(), OptProfileSelector::kMAX,
                           Dims3(1, 256, 1024));
    tensorrt_log::gLogVerbose << "optimization profile is: " << profile << std::endl;
    ...
    tensorrt_log::gLogVerbose << "build serialized networkd" << std::endl;
...
bool SuperGlue::infer(const Eigen::Matrix<double, 259, Eigen::Dynamic> &features0,
    ...
    tensorrt_log::gLogVerbose << "start super glue infer!" << std::endl;
    if (!context_) {
    ...
    tensorrt_log::gLogVerbose << "set binding dim params: " << keypoints_0_index << ", 1, " << features0.cols() << ", 2" << std::endl;
    context_->setBindingDimensions(keypoints_0_index, Dims3(1, features0.cols(), 2));

    tensorrt_log::gLogVerbose << "set binding dim params: " << scores_0_index << ", 1, " << features0.cols() << std::endl;
    context_->setBindingDimensions(scores_0_index, Dims2(1, features0.cols()));

    tensorrt_log::gLogVerbose << "set binding dim params: " << descriptors_0_index << ", 1, 256, " << features0.cols() << std::endl;
    context_->setBindingDimensions(descriptors_0_index, Dims3(1, 256, features0.cols()));

    tensorrt_log::gLogVerbose << "set binding dim params: " << keypoints_1_index << ", 1, " << features1.cols() << ", 2" << std::endl;
    context_->setBindingDimensions(keypoints_1_index, Dims3(1, features1.cols(), 2));

    tensorrt_log::gLogVerbose << "set binding dim params: " << scores_1_index << ", 1, " << features1.cols() << std::endl;
    context_->setBindingDimensions(scores_1_index, Dims2(1, features1.cols()));

    tensorrt_log::gLogVerbose << "set binding dim params: " << descriptors_1_index << ", 1, 256, " << features1.cols() << std::endl;
    ...
    tensorrt_log::gLogVerbose << "copy to super glue." << std::endl;
...
bool SuperGlue::deserialize_engine() {
    ...
    if (file.is_open()) {
        ...
        delete [] model_stream;
        tensorrt_log::gLogVerbose << "read super glue engine file" << std::endl;
        return true;
    }
    return false;
}
