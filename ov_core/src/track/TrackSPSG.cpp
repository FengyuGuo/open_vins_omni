...
TrackSPSG::TrackSPSG(std::unordered_map<size_t, std::shared_ptr<CamBase>> cameras, int numfeats, int numaruco, bool stereo,
                           ...
                           weight_dir(weight_dir), config_path(config_path), superglue_type(superglue_type)
{
  //superpoint superglue init stuff
  Configs config(config_path, weight_dir);
  superpoint = std::make_shared<SuperPoint>(config.superpoint_config);
  superpoint->build();
  superglue = std::make_shared<SuperGlue>(config.superglue_config);
  superglue->build();
}

void TrackSPSG::feed_new_camera(const CameraData &message) {
...
void TrackSPSG::feed_monocular(const CameraData &message, size_t msg_id) {
  ...
  if (pts_last.find(cam_id) == pts_last.end() || pts_last[cam_id].empty()) {
    SUPERPOINT_OUTPUT feature0;
    std::vector<cv::KeyPoint> good_left;
    std::vector<size_t> good_ids_left;
    perform_detection_monocular(img, mask, good_left, feature0, good_ids_left);
    std::lock_guard<std::mutex> lckv(mtx_last_vars);
    img_last[cam_id] = img;
    img_mask_last[cam_id] = mask;
    pts_last[cam_id] = good_left;
    ids_last[cam_id] = good_ids_left;
    last_frame_features[cam_id] = feature0;
    return;
  }

  // Our new keypoints and descriptor for the new image
  std::vector<cv::KeyPoint> pts_new;
  SUPERPOINT_OUTPUT feature_new;
  std::vector<size_t> ids_new;

  // First, extract new descriptors for this new image
  perform_detection_monocular(img, mask, pts_new, feature_new, ids_new);
  ...
  perform_matching(pts_last[cam_id], pts_new, last_frame_features[cam_id], feature_new, cam_id, cam_id, matches_ll);
  ...
  for (size_t i = 0; i < pts_new.size(); i++) {
    ...
    for (size_t j = 0; j < matches_ll.size(); j++) {
      if (matches_ll[j].trainIdx == (int)i) {
        idll = matches_ll[j].queryIdx; // queryIdx: feature index in last frame. trainIdx: feature index in current frame.
      }
    }
  ...
  // Move forward in time
  {
    ...
    pts_last[cam_id] = good_left;
    ids_last[cam_id] = good_ids_left;
    last_frame_features[cam_id] = feature_new;
  }
...
void TrackSPSG::feed_stereo(const CameraData &message, size_t msg_id_left, size_t msg_id_right) {
  PRINT_WARNING("stereo feature track with superpoint and superglue is not implemented yet!\n");
#if 0
  // Start timing
  rT1 = boost::posix_time::microsec_clock::local_time();
...
void TrackSPSG::perform_detection_monocular(const cv::Mat &img0, const cv::Mat &mask0, std::vector<cv::KeyPoint> &pts0, SUPERPOINT_OUTPUT &feat0, std::vector<size_t> &ids0) 
{
  // Assert that we need features
  assert(pts0.empty());

  // Extract our features (use superpoint)
  superpoint->infer(img0, feat0);

  std::vector<cv::KeyPoint> pts0_ext;
  for(int i = 0; i < feat0.cols(); i++)
  {
    size_t temp = ++currid;
    cv::KeyPoint kpt;
    kpt.pt.x = feat0(1, i);
    kpt.pt.y = feat0(2, i);
    kpt.response = feat0(0, i);
    pts0.push_back(kpt);
    ids0.push_back(temp);
  }
}

void TrackSPSG::perform_detection_stereo(const cv::Mat &img0, const cv::Mat &img1, const cv::Mat &mask0, const cv::Mat &mask1,
...
void TrackSPSG::perform_matching(std::vector<cv::KeyPoint> &pts0, std::vector<cv::KeyPoint> &pts1, 
  SUPERPOINT_OUTPUT &feat0, SUPERPOINT_OUTPUT &feat1, size_t id0, size_t id1,
  std::vector<cv::DMatch>& matches)
{
  ...
  {
    int last_idx = raw_match[i].queryIdx;
    int cur_idx = raw_match[i].trainIdx;

    cv::Point2f pt;
    pt.x = feat0(1, last_idx);
    pt.y = feat0(2, last_idx);
    
    cv::Point2f pt_n = camera_calib.at(id0)->undistort_cv(pt);
    pts_n_0.push_back(pt_n);

    pt.x = feat1(1, cur_idx);
    pt.y = feat1(2, cur_idx);

    pt_n = camera_calib.at(id1)->undistort_cv(pt);
    pts_n_1.push_back(pt_n);
  }
  std::vector<uchar> out_mask;
  cv::findFundamentalMat(pts_n_0, pts_n_1, cv::FM_RANSAC, 1.0, 0.999, out_mask);
  ...
  for(int i = 0; i < out_mask.size(); i++)
  {
    if(out_mask[i] != 0)
    {
      good_match.push_back(raw_match[i]);
    }
  }
  PRINT_DEBUG("reject outliers from superglue. raw match: %u, good match: %u\n", raw_match.size(), good_match.size());
  // matches = raw_match;
  matches = good_match;
}
