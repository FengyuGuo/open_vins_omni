...
//!
    ...
    public:
        ...
        BufferManager(std::shared_ptr<nvinfer1::ICudaEngine> engine, const int batchSize = 0,
            ...
            tensorrt_log::setReportableSeverity(nvinfer1::ILogger::Severity::kINFO);
            // Full Dims implies no batch size.
            assert(engine->hasImplicitBatchDimension() || mBatchSize == 0);
            // Create host and device buffers
            tensorrt_log::gLogVerbose << mEngine->getNbBindings() << " buffers will be created!" << std::endl;
            for (int i = 0; i < mEngine->getNbBindings(); i++) {
                ...
                tensorrt_log::gLogVerbose << vol << " * " << int(type) << " buffer will be created!" << std::endl;
        ...
        void copyInputToDevice() {
            tensorrt_log::gLogVerbose << "start to copy to buffer" << std::endl;
    ...
    private:
        ...
        memcpyBuffers(const bool copyInput, const bool deviceToHost, const bool async, const cudaStream_t &stream = 0) {
            tensorrt_log::gLogVerbose << "copy buffer, device to host? " << deviceToHost << std::endl;
            tensorrt_log::gLogVerbose << "NbBindings: " << mEngine->getNbBindings() << std::endl;
            for (int i = 0; i < mEngine->getNbBindings(); i++) {
                void *dstPtr
                        = deviceToHost ? mManagedBuffers[i]->hostBuffer.data() //crash point
                                       : mManagedBuffers[i]->deviceBuffer.data();
                tensorrt_log::gLogVerbose << "dst ptr: " << dstPtr << std::endl;
                const void *srcPtr
                        = deviceToHost ? mManagedBuffers[i]->deviceBuffer.data()
                                       : mManagedBuffers[i]->hostBuffer.data();
                tensorrt_log::gLogVerbose << "src ptr: " << srcPtr << std::endl;
                const size_t byteSize = mManagedBuffers[i]->hostBuffer.nbBytes();
                tensorrt_log::gLogVerbose << "copy from device to host? " << deviceToHost << std::endl;
                const cudaMemcpyKind memcpyType = deviceToHost ? cudaMemcpyDeviceToHost : cudaMemcpyHostToDevice;
                tensorrt_log::gLogVerbose << "start to copy data! size: " << byteSize << std::endl;
                // tensorrt_log::gLogWarning << __FILE__ << ":" << __LINE__ << ": start to copy data! size: " << byteSize;
                // LOG(INFO) << "This is an INFO log.";
                if ((copyInput && mEngine->bindingIsInput(i)) || (!copyInput && !mEngine->bindingIsInput(i))) {
                    if (async)
                    {
                        auto ret = cudaMemcpyAsync(dstPtr, srcPtr, byteSize, memcpyType, stream);
                        tensorrt_log::gLogVerbose << "copy mem result: " << ret << std::endl;
                    }
